...
class Extract_Embeddings():
    ...
    # def compute_and_print_cosine_similarities(self):
    #     """Compute and print cosine similarities between all pairs of face encodings."""
    #     num_encodings = len(self.face_encodings)
    #     if num_encodings == 0:
    #         print("No face encodings available to compute similarities.")
    #         return
        
    #     for i in range(num_encodings):
    #         for j in range(i + 1, num_encodings):
    #             similarity = cosine_similarity(self.face_encodings[i], self.face_encodings[j])
    #             print(f"Cosine similarity between encoding {i} and {j}: {similarity}")

# Example usage
rootdir = os.getcwd()  # Get current working directory
extractor = Extract_Embeddings(rootdir)
extractor.extract_and_save_embeddings()
# extractor.compute_and_print_cosine_similarities()
