...
def get_excel_file_path() -> str:
    try:
        ...
        return 'emo.xlsx'  # Fallback to a default path

def mark_emo(emo: str, file_path: str = '', sheet_name: str = 'Sheet'):
    """
    Record the emotion and the current time in the Excel file.

    Args:
        emo (str): Emotion label to write.
        file_path (str): Path to Excel file (default is 'emo.xlsx').
    ...
    try:
        ...
        # Select existing sheet or create a new one
        if sheet_name in workbook.sheetnames:
            sheet = workbook[sheet_name]
        else:
            sheet = workbook.create_sheet(sheet_name)

        # Determine the next row for writing
        ...
        # Write emotion, time, and date to the Excel sheet
        sheet.cell(row=next_row, column=1, value=emo)
        sheet.cell(row=next_row, column=2, value=time_att)
        sheet.cell(row=next_row, column=3, value=date_att)

        # Save the workbook
        workbook.save(file_path)
        print(f"Emotion '{emo}' and time '{time_att}' have been recorded in {file_path} on sheet '{sheet_name}'.")
...
def analysis(
    ...
    Returns:
        None
    """
    print("Indexing database "+db_path)
    ...
    files = [f for f in files if os.path.isfile(source_dir+'/'+f)]
    print(*files, sep="\n")
    for file in files:
        ...
        img_path = os.path.join(dest_dir, file)
...
def perform_facial_recognition(
    ...
    for idx, (x, y, w, h, is_real, antispoof_score) in enumerate(faces_coordinates):
        ...
        print(target_label, x, y, w, h, is_real, antispoof_score)
...
def overlay_emotion(
    ...
    # # Print the highest score emotion
    # highest_emotion = emotion_df.iloc[0]
    # print(f"Highest emotion: {highest_emotion['emotion']} with score: {highest_emotion['score']}")

    # Get the highest score emotion and log it to Excel
    highest_emotion = emotion_df.iloc[0]["emotion"]
    print(highest_emotion)
    mark_emo(highest_emotion)
            ...
            if text_location_y <= y + h:
                ...
                cv2.rectangle(
                    ...
                    (
                        ...
                        y + 13 + (index + 1) * 20, # type: ignore
                    ),
                    (
                        x - IDENTIFIED_IMG_SIZE + 70 + bar_x,
                        y + 13 + (index + 1) * 20 + 5, # type: ignore
                    ),
                    (255, 255, 255),
                    cv2.FILLED,
                )
